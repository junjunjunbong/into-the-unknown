...
- **What counts as settled**: an entry settles when the user makes the decision, or explicitly delegates it (record `(delegated)` — still their call, so it settles). Settled means the *user* said or confirmed it — writing it into a report does not count.
