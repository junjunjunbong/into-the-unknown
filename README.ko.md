...
### 원장(ledger): unknown이 실제로 닫히는 걸 지켜보기

이 플러그인의 진행률 표시줄은 `.unknowns/ledger.md`입니다: 모든 unknown이 번호 붙은 항목(`U1`, `U2`, …)이 되어 생애주기를 갖고, **모든 스킬은 닫힘 의식(closing ritual)으로 끝납니다** — `Closed: U2 (criterion 발화) · Opened: U9 · Still open: U3 (기본값: last-write-wins)`.

닫힘 규칙은 일부러 엄격합니다: 항목은 **당신의** 발화로만 닫힙니다 — 당신이 내린 결정, 당신이 말로 표현한 취향, 당신이 통과한 teach-back(`/blindspot`은 리포트를 쓰고 나서 그 내용으로 당신에게 확인 질문을 합니다). Claude가 문서에 답을 적는 것은 닫힘이 아닙니다 — 그건 unknown을 당신 머리에서 파일로 옮겼을 뿐입니다. "내 unknown이 채워지고 있다"는 느낌은 정확히, 전에는 말 못 하던 걸 이제 말할 수 있게 되어 항목이 ✅로 뒤집히는 그 순간입니다.
